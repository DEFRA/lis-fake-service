      ...
      rejected: [
        {
          ...
          causes: [{ code: 'X001', desc: 'Foo is bad', sev: 'e', field: 'Foo' }]
        }
      ]
    }
  })
...
test('submissions for different receipts are validated independently', () => {
  ...
  // Act
  const firstReceiptNum = store.submit({ txnId: 'txn-1', rows: [acceptedRow] })
  const secondReceiptNum = store.submit({ txnId: 'txn-2', rows: [rejectedRow] })
