...
/**
 * Handles a decoded Register_Births_Asynchronous payload: validates the
 * CTS_OL_User credentials, submits the batch to the birth store for later
 * polling, and returns a receipt. Throws a DomainError for any
...
export function handle(innerXml) {
  ...
  try {
    payload = parseRegisterBirthsRequest(innerXml)
  } catch {
    throw new DomainError(
      'CTWS000',
      'Malformed Register_Births_Asynchronous payload'
    )
  }
  ...
  if (birthStore.hasSubmission(payload.username, payload.txnId)) {
