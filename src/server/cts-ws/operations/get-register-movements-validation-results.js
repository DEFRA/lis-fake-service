 ...
 * validates the CTS_OL_User credentials, then polls the movement store for
 * the receipt's results - CTWS002 for an unknown receipt, CTWS806 while the
 * store's random validation delay hasn't elapsed yet, otherwise the
...
export function handle(innerXml) {
  ...
  const submission = movementStore.retrieveResults(payload.receiptNum)
