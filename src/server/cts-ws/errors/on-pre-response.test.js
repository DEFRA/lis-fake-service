...
const { onPreResponse } = await import('./on-pre-response.js')
const { decodeDataPayload } = await import('../xml/soap-envelope.js')
const { DomainError } = await import('./domain-error.js')
const { MalformedRequestError } = await import('./malformed-request-error.js')
const { TransportFaultError } = await import('./transport-fault-error.js')
...
test('it renders a SystemException for a DomainError, wrapped in a 200 envelope', () => {
  // Arrange
  const request = { response: new DomainError('CTWS810', 'Holding not found') }
  const h = makeH()
